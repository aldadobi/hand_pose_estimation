...
def visualize(original_img,skeleton,mesh):
    res_img = original_img.copy()
    res_img = np.concatenate((res_img,skeleton,mesh), axis=1)
    return res_img
...
webcam = cv2.VideoCapture(1) # webcam의 상태에 따라 0넣어주시면 됩니다.
cur_frame = 0
while(True):
    retval, img_original_bgr = webcam.read()
    #img_original_rgb = img_original_bgr[:,:,::-1].copy()
    
    #file_name = img_path.split('/')[-1][:-4]
    file_name = f'{cur_frame:05d}'
    img_path = osp.join(args.img_path,'frames',file_name+'.jpg')
    if retval:
        if args.save_frame:
    ...
    original_img = img_original_bgr[:,:,::-1].copy()
    ...
    res_img = visualize(vis_box/255, vis_skeleton/255, render_out[0].cpu().numpy()/255)
    ImShow(res_img)
